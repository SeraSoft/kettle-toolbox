/******************************************************************************
 *
 * File:    StringResizeDynamicStepMeta.java
...
@Step(
        id = "SeraStringResizeDynamicStep",
        image = "org/serasoft/di/steps/stringresizedynamic/resources/demo.svg",
        name = "StringResizeDynamic.Step.Name",
        description = "StringResizeDynamic.Step.TooltipDesc",
        i18nPackageName = "org.serasoft.di.steps.stringresizedynamic",
        categoryDescription = "StringResizeDynamic.Category"
)
@InjectionSupported(localizationPrefix = "StringResizeDynamic.Step.Injection.")
public class StringResizeDynamicStepMeta extends BaseStepMeta implements StepMetaInterface {
     ...
     */
    private StringItemToResize[] itemsToResize;
     ...
     */
    public StringResizeDynamicStepMeta() {
        super();
        fieldStream = new String[0];
        itemsToResize = new StringItemToResize[0];
    }
     ...
     */
    public StringItemToResize[] getItemsToResize() {
        return itemsToResize;
    }

    public void allocate(int nrRows) {
        fieldStream = new String[nrRows];
        itemsToResize = new StringItemToResize[nrRows];
    }
    ...
    public StepDialogInterface getDialog(Shell shell, StepMetaInterface meta, TransMeta transMeta, String name) {
        return new StringResizeDynamicStepDialog(shell, meta, transMeta, name);
    }
    ...
    public StepInterface getStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, TransMeta transMeta,
                                 Trans disp) {
        return new StringResizeDynamicStep(stepMeta, stepDataInterface, cnr, transMeta, disp);
    }
    ...
    public StepDataInterface getStepData() {
        return new StringResizeDynamicStepData();
    }
     ...
     */
    public StringResizeDynamicStepMeta clone() {
        StringResizeDynamicStepMeta retval = (StringResizeDynamicStepMeta) super.clone();

        int nrRows = fieldStream.length;
        retval.allocate(nrRows);
        System.arraycopy(fieldStream, 0, retval.fieldStream, 0, nrRows);
        System.arraycopy(itemsToResize, 0, retval.itemsToResize, 0, nrRows);
    ...
    public String getXML() throws KettleValueException {
        ...
        retval.append("    <fields_to_resize>").append(Const.CR);

        for (int i = 0; i < itemsToResize.length; i++) {
            retval.append("    <field_to_resize>").append(Const.CR);
            retval.append("     ").append(XMLHandler.addTagValue("field_name", itemsToResize[i].getFieldname()));
            retval.append("     ").append(XMLHandler.addTagValue("ref_table_name", itemsToResize[i].getTableName()));
            retval.append("     ").append(XMLHandler.addTagValue("ref_column_name", itemsToResize[i].getColumnName()));
            retval.append("    </field_to_resize>").append(Const.CR);
        }
        retval.append("    </fields_to_resize>").append(Const.CR);
    ...
    public void loadXML(Node stepnode, List<DatabaseMeta> databases, IMetaStore metaStore) throws KettleXMLException {
        try {
            ...
            Node fields = XMLHandler.getSubNode(stepnode, "fields_to_resize");
            int count = XMLHandler.countNodes(fields, "field_to_resize");

            allocate(count);

            for (int i = 0; i < count; i++) {
                Node fnode = XMLHandler.getSubNodeByNr(fields, "field_to_resize", i);
                itemsToResize[i] = new StringItemToResize(XMLHandler.getTagValue(fnode, "field_name"),
    ...
    public void saveRep(Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step)
        ...
        try {
            ...
            for (int i = 0; i < itemsToResize.length; i++) {
                rep.saveStepAttribute(id_transformation, id_step, i, "item_to_resize_field_name", getNullOrEmpty(itemsToResize[i].getFieldname()));
                rep.saveStepAttribute(id_transformation, id_step, i, "item_to_resize_ref_table_name", getNullOrEmpty(itemsToResize[i].getTableName()));
                rep.saveStepAttribute(id_transformation, id_step, i, "item_to_resize_ref_column_name", getNullOrEmpty(itemsToResize[i].getColumnName()));
            }
        } catch (Exception e) {
    ...
    public void readRep(Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> databases)
        ...
        try {
            ...
            for (int i = 0; i < nrfields; i++) {
                itemsToResize[i] = new StringItemToResize(rep.getStepAttributeString(id_step, i, "field_name"),
    ...
    public void getFields(RowMetaInterface rowMeta, String name, RowMetaInterface[] info, StepMeta nextStep,
        ...
        resizeValueMetaForNeededStrings(rowMeta, name);
    }

    public void retrieveTableFieldsMetadata(List<StringItemToResize> itemsToResize) {
        ...
        try {
            ...
            for (StringItemToResize item : itemsToResize) {
    ...
    public void resizeValueMetaForNeededStrings(RowMetaInterface rowMeta, String name) {

        List<StringItemToResize> fieldsToProcessList = Arrays.asList(itemsToResize);
        retrieveTableFieldsMetadata(fieldsToProcessList);
        try {
            // This is the value we need to
            for (StringItemToResize item : fieldsToProcessList) {
                int fieldPos = rowMeta.indexOfValue(item.getFieldname());
                if (fieldPos == -1) {
                    // TODO Field not present in flow: needs to be managed
                }
                ValueMetaInterface vm = rowMeta.getValueMeta(fieldPos);
                vm = ValueMetaFactory.createValueMeta(item.getFieldname(), ValueMetaInterface.TYPE_STRING);
                rowMeta.setValueMeta(fieldPos, vm);
                vm.setLength(item.getTargetSize());
